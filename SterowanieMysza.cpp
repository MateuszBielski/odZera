...
bool SterowanieMysza::on_button_press_eventNowe(GdkEventButton* event)
{
    ...
    if(event->button == 1 && event->type == GDK_2BUTTON_PRESS)  //podwójne kliknięcie lewy
    {
        auto klawiszFunkcyjny = event->state;
        WybieranieModelu(klawiszFunkcyjny);
    }
    
    if(event->button == 2 && event->type == GDK_2BUTTON_PRESS)  //podwójne klik prawy
    {       
    }
    OdswiezWidok();
    return true;
...
bool SterowanieMysza::on_motion_notify_eventNowe(GdkEventMotion* event)
{
...
void SterowanieMysza::PobierzWspolrzedneWruchu_ix_iy_x_y(gdouble x, gdouble y)
{
    x = x;
    y = y;
    ix = static_cast<int>(x);
